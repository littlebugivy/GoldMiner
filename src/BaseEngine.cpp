...
{
	m_ppDisplayableObjects.insert(std::pair<int, DisplayableObject*>(iIndex, pObject));
}
...
{
	std::unordered_map<int, DisplayableObject*>::iterator it;
	m_iDrawableObjectsChanged = 0;
	if ( !m_ppDisplayableObjects.empty() )
	{
		for ( it = m_ppDisplayableObjects.begin(); it != m_ppDisplayableObjects.end(); ++it)
			{
			if (it->second != NULL){
				(it->second)->DoUpdate(iCurrentTime);
		...
		{ // Then points are in clockwise order so central one can be eliminated as long as we don't
			...
			{// If not then try the next position
				...
				DrawTriangle( pXArray[i1], pYArray[i1], pXArray[i2], pYArray[i2], 
							pXArray[i3], pYArray[i3], /*
													  (iPoints)*/uiColour, pTarget );
...
void BaseEngine::NotifyAllObjects( int iSignalNumber )
{
	...
	if (!(m_ppDisplayableObjects).empty())
	{
		...
		{
			if ((it->second) != NULL){
				(it->second)->Notify(iSignalNumber);
			}
		}
	}
}
...
int BaseEngine::NotifyAllObjectsGetCountNonZero( int iSignalNumber )
{
	...
	if (!(m_ppDisplayableObjects).empty())
	{
		for (it = (m_ppDisplayableObjects).begin(); it != (m_ppDisplayableObjects).end(); ++it)
		{
			if ((it->second)->Notify(iSignalNumber) != 0 && (it->second) != NULL)
				iReturn++;
		}
	}
	return iReturn;
...
int BaseEngine::NotifyAllObjectsGetSum( int iSignalNumber )
{
	...
	if (!(m_ppDisplayableObjects).empty())
	{
		...
		{
			if ((it->second) != NULL){
				iReturn += (it->second)->Notify(iSignalNumber);
			}
		}
	}
	return iReturn;
...
int BaseEngine::NotifyAllObjectsGetMax( int iSignalNumber )
{
	...
	if (!(m_ppDisplayableObjects).empty())
	{
		...
		{
			if ((it->second) != NULL){
				int ival = (it->second)->Notify(iSignalNumber);
				if (ival > iReturn)
					iReturn = ival;
			}
		}
